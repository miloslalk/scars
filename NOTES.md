...
- Note (2026-07-28 audit): Tapping outside the body + "Reflect" now records the point (was a no-op), but the `07_outside_the_body` clip still never plays — deciding whether Save should launch it for the `outside` region is an open product question. `00_intro`/`26_outro` clips are bundled but unreferenced by code.
